...
(function () {
    ...
    * Константы
    * */
    const FAST_RESULT_TEMPLATE = document.getElementById('search-fast-results-item').content.querySelector('.js-fast-result-item-template');
    ...
    let _createFastResultFragment = function (resultsObj) {
        if (resultsObj !== undefined) {
            ...
            resultsObj.item.items.catalogItems.forEach(function (result) {
                ...
                link.querySelector('.js-fast-result-price').textContent = result.price + ' ' + result.currency;
